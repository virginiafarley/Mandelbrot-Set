...
When the program is running, you can recenter the display to a new location by using the <kbd>Mouse</kbd> to click on on any point in the display.  The arrow keys <kbd>↑</kbd>, |<kbd>↓</kbd>, <kbd>←</kbd>, and |<kbd>→</kbd> allow you to move up, down, left, and right, respectively.  The *WASD* keys provide the same functionality.

<p align="center">
<img src="images/mandelbrot2.png" alt="mandelbrot2" width="500"/>
</p>

To zoom into and out of the display, use the <kbd>+</kbd> and <kbd>-</kbd> keys.

<p align="center">
<img src="images/mandelbrot3.png" alt="mandelbrot3" width="500"/>
</p>
...
│   └── mandelbrot.h
│   └── pixel_vector.cpp #  data structure that stores pixels
│   └── pixel_vector.h
│   └── barrier.cpp # thread synchronization barrier
│   └── barrier.h
...
| Object Oriented Programming | The project uses Object Oriented Programming techniques.  | The project code is organized into classes with class attributes to hold the data, and class methods to perform tasks (e.g. **pixel.h**, **pixel_vector.h**, **barrier.h**, **display.h**, and **window.h**). |
...
| Object Oriented Programming | Class constructors utilize member initialization lists.  | All class members that are set to argument values are initialized through member initialization lists (e.g. **pixel.h**, **pixel_vector.h**, **barrier.h**, **display.h**, and **window.h**).    |
| Object Oriented Programming | Classes abstract implementation details from their interfaces.  | All class member functions document their effects via both function names and comments. Member functions do not change program state unless documented (e.g. all class setters and ***wait*** member function in **barrier** class). |
...
| Memory Management | The project uses destructors appropriately.  | The **display** class modifies state upon termination of an object via a destructor.  The **pixel_vector** class joins threads in its destructor.  |
...
| Memory Management | The project uses move semantics to move data, instead of copying it, where possible.  | Please refer to the **pixel vector** constructor as an example.  |
| Memory Management | The project uses smart pointers instead of raw pointers.  | A shared pointer named *fractal* is declared and initialized in the main function.  The display object also owns this resource. The pixel vector hold a non-owned data handle to this object.  The pixel vector contains two unique pointer member variables of type *Barrier*.|
| Concurrency | The project uses multithreading.  | Multiple threads are used in execution, implemented in **pixel vector**.  |
| Concurrency | A promise and future is used in the project.  | n/a  |
| Concurrency | A mutex or lock is used in the project.  | A mutex and unique lock is used in the **barrier** class.  |
| Concurrency | A condition variable is used in the project.  | A conditional variable is used in the **barrier** class's member function *wait* to synchronize thread execution.  |
